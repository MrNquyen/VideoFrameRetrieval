    ...
    def batch_caption(self, images, prompt):
        ...
        def parse(response):
            ...
            if len(parts) != batch_size:
                ValueError(f"Response didn't provide equal captions: {len(parts)} != {batch_size}")
            return parts

        generation_config = dict(max_new_tokens=1024, do_sample=False)
        pixel_values = [self.tokenize_image(image) for image in images]
        ...
        prefix = "".join([prefix_template.format(id=i+1) for i in range(batch_size)])
...
class ImageCaptionerInternVL3:
    ...
    def build_transform(self):
        self.transform = Transform(image_size=448).transform_from_PIL()
